import { r as rating } from './data-922c8b52.js';
import { u, h as createCommonjsModule, j as commonjsGlobal, k as commonjsRequire, l as chrome, o as onNodesAvailable, t as toFloat, e as getCurrencyPrefix, c as toDays, f as today, d as toDate } from './util-0f82c846.js';
import { l as localization$1 } from './index-5306fdc6.js';
...
function loanInvBreakdownLegend (model, node)
{
    render(html`
...
function $createHeader ()
{
    return html`
        <tr>
            <th></th>
            <th style="text-align: left" >${localization$1('Name')}   </th>
            <th style="text-align: right">${localization$1('Percent')}</th>
            <th style="text-align: right">${localization$1('Amount')} </th>
        </tr>
    `;
}
...
function renderRatings (model, node)
{
    ...
    ${[...node.children]}
    ${createOriginatorRow("Mintos' " + localization$1('Rating'), mintosRating(model.mintosRating))}
    ${createOriginatorRow(html`
ExploreP2P's <a href="https://explorep2p.com/mintos-lender-ratings/" target="_blank">
${localization$1('Rating')}
...
function renderExtraDetails (model, settings, node)
{
    const warnings = {};
    const details = {};
    ...
    if (settings.LoanShowPaymentWarning && model.ontimePercent < 60.0)
        warnings.Payments = model.ontimePercent.toFixed(2) + '% ' + localization$1('Ontime');
    
    if (model.borrower.age && model.borrower.age > 55)
        warnings.Age = `${model.borrower.age} ${localization$1('Years')}`;
    
    if (model.borrower.unemplyed)
        warnings['$Occupation'] = 'Unemployed';
    
    if (settings.LoanShowTotalGraceTime)
        details.TimeInGrace = `${model.graceDays} ${localization$1('Days')}`;
    ...
    if (settings.LoanShowNextPaymentRow)
        details.NextPayment = Number.isNaN(model.nextPayment) ? 'N/A' :
            model.nextPayment >= 0
                ? `${model.nextPayment} ${localization$1('Days')}`
                : `${-model.nextPayment} ${localization$1('DaysLate')}`;
    
    function country ()
    {
        if (model.country)
            return createDetailsRow(localization$1('Country'), model.country)
    }
    
    render(html`
    ${Object.entries(warnings).map(([key, value]) => createDetailsRowWarning(localization$1(key), value))}
    ${country()}
    ${[...node.children]}
    ${Object.entries(details).map(([key, value]) => createDetailsRow(localization$1(key), value))}
    `, node);
...
{
    return createDetailsRow(`${localization$1('Warning')}: ${header}`, content, 'invext-details-warning');
}
 ...
 * @param {Map} options
 */
function chart (type, data, options, target = undefined)
{
    data = data || {};
    options = options || {};
 ...
 */
function latePaymentHistogramChart (id, data, target = null)
{
...
class Payment
{
    ...
    get paymentDate ()
    {
        ...
        {
            const cell = this.row.querySelector(`td[data-m-label="${localization$1('$PaymentDate')}"]`);
    ...
    get scheduledDate ()
    {
        ...
        {
            const cell = this.row.querySelector(`td[data-m-label="${localization$1('$Date')}"]`);
            const text = cell ? cell.innerText : null;
    ...
    get isLate ()
    {
        return this.paymentStatus.includes(localization$1('$Late'));
    }
    
    get isScheduled ()
    {
        return this.paymentStatus === localization$1('$Scheduled');
    }
    
    get isPaid ()
    {
        return this.paymentStatus === localization$1('$Paid');
    }
    
    get isPaidPartially ()
    {
        return this.paymentStatus === localization$1('$PartiallyPaid');
    }
}
...
function parseSchedule (schedule, details, model)
{
    ...
    if (nextPaymentDate && ![
        localization$1('$Finished'),
        localization$1('$FinishedPrematurely'),
        localization$1('$Default')
    ].includes(loanStatus))
...
function parseAge (borrower, model)
{
    ...
    {
        let match;
        if (row.firstChild.innerText === localization$1('$Borrower') && (match = row.lastChild.innerText.match(localization$1('$BorrowerPattern'))))
        {
...
function parseUnemployed (borrower, model)
{
    for (const row of borrower.querySelectorAll('tr'))
    {
        ...
        if (key === localization$1('$Occupation') && value === 'unemployed')
        {
