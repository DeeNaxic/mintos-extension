...
2. If you prefer a more hands-on and higher transparency approach, you are more than welcome to perform a manual install from the source code. You can download the source code directly from this repository and perform a full audit. If you are satisfied you can then [build](#Building) the extension, open Chrome to the [extensions page](chrome://extensions/) and load it in from the dist folder, as an unpacked extension. This way is a bit more difficult to set up, and disables automatic updates - but we did want to provide this method for better security.

<br>

### Building

Starting with version 2.0.0 the extension requires [Node 14](https://nodejs.org/) and [node-gyp](https://github.com/nodejs/node-gyp) *to build*. Please follow their installation instructions if you're building the Investment Extension 2.0.0+ for the first time.
